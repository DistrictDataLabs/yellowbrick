...
class DecisionBoundariesVisualizerTest(VisualTestCase):
    ...
    def test_integrated_plot_numpy_named_arrays(self):
        ...
        y = iris_data.target
        # y = y.astype(dtype=[('species', 'O')], casting='unsafe')

        visualizer = DecisionBoundariesVisualizer(model, features=['sepal_length', 'sepal_width'])
        visualizer.fit(X, y=y)
        self.assertEquals(visualizer.features_, ['sepal_length', 'sepal_width'])
