...
class TopFeaturesVisualizer(ClassificationScoreVisualizer):
    ...
    * Add funcitonality for "one sided" visualizer that takes absolute values
    ...
    def __init__(self, model, ax=None, N=20, features=None, **kwargs):
        ...
        # Check to see if there are N features; otherwise replace with count of features
    ...
    def draw(self, **kwargs):
        ...
        # make sure N doesn't exceed the number of features in the dataset
        # This will only prevent the graph from erroring out, but it will look dumb
        if 2 * self.N > len(self.top_coefficients):
            self.N = int(len(self.top_coefficients)/2)
            warnings.warn(
                'Value of N exceeds number of features. Plotting all features instead.'
            )
        ...
        self.ax.set_xticks(np.arange(2 * self.N))
    ...
    def finalize(self, **kwargs):
        ...
        self.set_title('Top {N} features for {class_label}'.format(N=self.N,
                                                                   class_label=self.class_label))
        self.ax.set_xticklabels(self.top_features, rotation=90, ha='left')
