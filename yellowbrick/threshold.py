...
class ThresholdVisualizer(ModelVisualizer):
    ...
    def __init__(self, model, colormap=None, color=None, n_trials=50, test_size_percent=0.1, quantiles=[0.1, 0.5, 0.9], random_state=0, title=None, **kwargs):
    ...
    def fit(self, X, y=None, **kwargs):
        ...
        for trial in range(self.n_trials):
            train_X, test_X, train_y, test_y = train_test_split(X, y, test_size=self.test_size_percent, random_state=self.random_state)
    ...
    def draw(self, *kwargs):
        ...
        if self.colormap is not None or self.color is not None:
            color_values = resolve_colors(num_colors=3, colormap=self.colormap, color=self.color)
        else:
        ...
        lower_precision, median_precision, upper_precision = mstats.mquantiles(uniform_precision_plots, self.quantiles, axis=0)
        lower_recall, median_recall, upper_recall = mstats.mquantiles(uniform_recall_plots, self.quantiles, axis=0)
        lower_queue_rate, median_queue_rate, upper_queue_rate = mstats.mquantiles(uniform_queue_rate_plots, self.quantiles, axis=0)

        self.ax.plot(uniform_thresholds, median_precision, color=precision_color)
        self.ax.plot(uniform_thresholds, median_recall, color=recall_color)
        self.ax.plot(uniform_thresholds, median_queue_rate, color=queue_rate_color)

        self.ax.fill_between(uniform_thresholds, upper_precision, lower_precision, alpha=0.5, linewidth=0, color=precision_color)
        self.ax.fill_between(uniform_thresholds, upper_recall, lower_recall, alpha=0.5, linewidth=0, color=recall_color)
        self.ax.fill_between(uniform_thresholds, upper_queue_rate, lower_queue_rate, alpha=0.5, linewidth=0, color=queue_rate_color)
    ...
    def finalize(self, **kwargs):
        ...
        leg = self.ax.legend(('precision', 'recall', 'queue_rate'), frameon=True, loc='best')
